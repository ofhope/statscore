...
describe("linear", () => {
  it("should correctly calculate linear regression for a perfect positive correlation", () => {
    ...
    expect(result.m).toBeCloseTo(1); // gradient
    expect(result.b).toBeCloseTo(1); // intercept
  ...
  it("should correctly calculate linear regression for a perfect negative correlation", () => {
    ...
    expect(result.m).toBeCloseTo(-1); // gradient
    expect(result.b).toBeCloseTo(6); // intercept
    expect(result.rSquared).toBeCloseTo(1);
  ...
  it("should correctly calculate linear regression for a horizontal line", () => {
    ...
    expect(result.m).toBeCloseTo(0); // gradient
    expect(result.b).toBeCloseTo(5); // intercept
    expect(result.rSquared).toBeCloseTo(1);
  ...
  it("should correctly calculate linear regression for scattered data", () => {
    ...
    expect(result.m).toBeCloseTo(0.99); // gradient
    expect(result.b).toBeCloseTo(0.15); // intercept
    expect(result.rSquared).toBeCloseTo(0.97, 2); // R^2
  ...
  it("should apply precision option correctly", () => {
    ...
    expect(result.rSquared).toBeCloseTo(1.0); // For perfectly linear data, R2 remains 1 even with rounding of intercept
  });
  ...
  it("should return an error on bad numerical values", () => {
    ...
    expect(result.message).toContain("Data point at index 1 contains non-finite values (2, NaN). Linear regression requires finite numerical inputs.");
  });
  ...
  it("should handle a large number of data points efficiently and accurately", () => {
    ...
    expect(result.rSquared).toBeGreaterThan(0.99); // R-squared should be very high
  });
  ...
  it("should have R-squared of 1 for a perfectly horizontal line where all y-values are identical", () => {
    ...
    expect(result.rSquared).toBeCloseTo(1); // R^2 should be 1 if the line perfectly fits the data, even if y has no variance
  });
});
